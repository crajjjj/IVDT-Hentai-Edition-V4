...
float[] Function GetEmptyAdjustments(int Position)
	...
	while Stage > 0
		...
		int outIndex = AdjIndex(Stage, kSchlong)
		int flagIndex = FlagIndex(Stage, kSchlong)

		; Safety checks to avoid array out-of-bounds
		if outIndex >= 0 && outIndex < Output.Length && flagIndex >= 0 && flagIndex < Flags.Length
			Output[outIndex] = Flags[flagIndex]
		else
			Log("Skipped invalid index: Output[" + outIndex + "], Flags[" + flagIndex + "]", "GetEmptyAdjustments")
		endif
	endwhile
	return Output
EndFunction
...
int[] Function PositionFlags(int[] Output, string AdjustKey, int Position, int Stage)
	...
	if i >= 0 && i + 2 < Flags.Length
		Output[0] = Flags[i]
		Output[1] = Flags[i + 1]
		Output[2] = Flags[i + 2]
	else
		Log("PositionFlags: FlagIndex out of bounds for Position=" + Position + ", Stage=" + Stage, "ERROR")
		Output[0] = 0
		Output[1] = 0
		Output[2] = 0
	endif
